...
urlpatterns = [
    ...
    path(
        "login/",
        LoginView.as_view(
            template_name="usercustom/login.html", redirect_field_name="next",
        ),
        name="login",
    ),
    path("logout", LogoutView.as_view(next_page="usercustom:login"), name="logout"),
    path("logoutmodal/", LogOutModalView.as_view(), name="logout-modal"),
    path("profile", login_required(ProfileView.as_view()), name="profile"),
    path("changepasword", login_required(ChangePasswordView), name="change-password"),
